...
// Memeriksa peran 'it' dengan aman untuk logika rute dinamis.
...
// --- PERBAIKAN DI SINI ---
// Computed property baru untuk memilih nama rute yang benar secara dinamis.
const laporanAbsensiRouteName = computed(() => {
    return isIT.value ? 'admin.laporan.absensi.index' : 'laporan.absensi.index';
});

// Computed property baru untuk mengecek apakah salah satu rute laporan sedang aktif.
...
// Menentukan apakah dropdown Manajemen harus ditampilkan (hanya untuk IT).
            ...
            <nav class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex">
                            ...
                            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                ...
                                <!-- --- PERBAIKAN DI SINI --- -->
                                <!-- Sekarang hanya ada satu NavLink untuk Laporan Absensi -->
                                <NavLink 
                                    v-if="canViewLaporanAbsensi" 
                                    :href="route(laporanAbsensiRouteName)" 
                                    :active="isLaporanAbsensiActive"
                                >
                                    Laporan Absensi
                                </NavLink>
                                
                                <NavLink v-if="canViewCatatanPelanggaran" :href="route('catatan-pelanggaran.index')" :active="route().current('catatan-pelanggaran.*')">
                                    Catatan Pelanggaran
                                </NavLink>

                                <NavLink v-if="canViewLiveMap" :href="route('live-map.index')" :active="route().current('live-map.index')">
                                    Live Map
                                </NavLink>

                                <!-- Dropdown Menu Manajemen (Hanya untuk IT) -->
                                <div v-if="showManajemenDropdown" class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <div :class="{ block: showingNavigationDropdown, hidden: !showingNavigationDropdown }" class="sm:hidden">
                    <div class="pt-2 pb-3 space-y-1">
                        ...
                        <ResponsiveNavLink v-if="canViewLaporanAbsensi" :href="route(laporanAbsensiRouteName)" :active="isLaporanAbsensiActive">
                            Laporan Absensi
                        </ResponsiveNavLink>
                        <ResponsiveNavLink v-if="canViewCatatanPelanggaran" :href="route('catatan-pelanggaran.index')" :active="route().current('catatan-pelanggaran.*')">
                            Catatan Pelanggaran
                        </ResponsiveNavLink>
                         <ResponsiveNavLink v-if="canViewLiveMap" :href="route('live-map.index')" :active="route().current('live-map.index')">
                            Live Map
                        </ResponsiveNavLink>
                    </div>
